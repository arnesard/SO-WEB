...
window.loadBarcodeMonstockData = function () {
    ...
    $.get("/oracle-fisik/barcode/data", function (response) {
        ...
        if (currentWh) {
            ...
            whSelect.val(""); // Pastikan tetap berada di opsi "⚠️ PILIH GUDANG"
        }
...
/**
 * 🔐 DELETE DB — Hanya Developer Authorized
 * Truncate: so_all_wh_barcode_monstock_db & so_all_wh_barcode_monstock_auto_db
 */
window.confirmDeleteDbMonstock = function () {
    Swal.fire({
        title: '🗑️ DELETE DATABASE',
        html: `
            <div class="text-start" style="font-size: 13px;">
                <div class="alert alert-danger py-2 px-3 mb-3" style="font-size: 11px; border-radius: 8px;">
                    <strong>⚠️ PERINGATAN KERAS!</strong><br>
                    Hanya <u>Developer</u> yang memiliki akses.<br>
                    Tindakan ini akan <strong>menghapus SELURUH data</strong> dari kedua tabel monstock secara permanen!
                </div>
                <label class="fw-bold text-secondary mb-1" style="font-size: 11px;">Masukkan Sandi Developer:</label>
                <input type="password" id="swal-dev-password"
                    class="form-control form-control-sm"
                    placeholder="••••••••••"
                    autocomplete="new-password"
                    style="letter-spacing: 3px; font-size: 14px;">
                <div class="mt-2 text-muted" style="font-size: 10px;">
                    Tabel yang akan di-truncate:<br>
                    → <code>so_all_wh_barcode_monstock_db</code><br>
                    → <code>so_all_wh_barcode_monstock_auto_db</code>
                </div>
            </div>
        `,
        ...
        confirmButtonText: 'EKSEKUSI TRUNCATE',
        cancelButtonText: 'BATALKAN',
        reverseButtons: true,
        preConfirm: () => {
            const pwd = document.getElementById('swal-dev-password').value.trim();
            if (!pwd) {
                Swal.showValidationMessage('❌ Sandi wajib diisi!');
                return false;
            }
            return pwd;
        }
        ...
        // ⏳ Loading pakai willOpen bukan onOpen
        Swal.fire({
            title: 'Mengeksekusi Truncate...',
            text: 'Sistem sedang membersihkan kedua tabel database monstock.',
            allowOutsideClick: false,
            showConfirmButton: false,
            willOpen: () => {
                Swal.showLoading();
        ...
        $.ajax({
            url: '/oracle-fisik/barcode/truncate-all',
            type: 'POST',
            ...
            success: function (res) {
                ...
                if (res.status === 'success') {
                    Swal.fire({
                        ...
                        html: `
                            <div style="font-size: 13px;">
                                <p class="mb-2">Password yang anda masukan <strong>benar</strong>.</p>
                                <div class="alert alert-success py-2 px-3" style="font-size: 11px; border-radius: 8px;">
                                    <strong>Data truncate berhasil dilakukan!</strong><br>
                                    DB berhasil dihapus bersih.
                                </div>
                                <div class="text-muted" style="font-size: 10px;">
                                    Tabel yang sudah di-truncate:<br>
                                    → <code>so_all_wh_barcode_monstock_db</code> ✓<br>
                                    → <code>so_all_wh_barcode_monstock_auto_db</code> ✓
                                </div>
                            </div>
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div style="font-size: 13px;">
                                <p class="mb-2">Password yang anda masukan <strong>salah</strong>.</p>
                                <div class="alert alert-danger py-2 px-3" style="font-size: 11px; border-radius: 8px;">
                                    Akses ditolak! Hanya developer yang berwenang.
                                </div>
                            </div>
                        `,
                        confirmButtonColor: '#dc3545',
                        ...
                        if (retry.isConfirmed) {
                            window.confirmDeleteDbMonstock();
                        }
                    });
                    ...
                    Swal.fire({
                        title: 'Error Sistem!',
                        text: res.message || 'Terjadi kesalahan sistem saat truncate.',
                        confirmButtonColor: '#dc3545'
                    });
                }
            },
            error: function () {
                ...
                Swal.fire({
                    ...
                    text: 'Tidak dapat terhubung ke server. Cek koneksi jaringan lu bro.',
                    confirmButtonColor: '#dc3545'
                });
            }
        });
